...
Eleven agent packages covering ingestion, contact enrichment, digest generation, outreach and analytics run on systemd timers with `OnFailure` units that email when something breaks. That last part is the whole difference between a script and a system: a script that fails silently at 3am is worse than no script.
...
Outbound email is CASL-compliant by construction, with a verified sending domain, physical-address footer, reply-to-unsubscribe, a suppression list checked before every send, and a daily send cap. Compliance is cheaper to build in than to retrofit.
...
Two known weaknesses I'd rather state than have found. Send state is currently split across two SQLite copies, laptop and VPS, because either can run outreach; the fix is a single authoritative database, and Postgres is already installed on the box. And there is no IMAP reply polling, because inbound volume was two emails. That is a scope call I'll defend until the volume changes.
